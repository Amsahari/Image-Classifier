...
def print_results(results_dic, results_stats_dic, model, 
    ...
    for key in results_stats_dic:
        if key.startswith('pct'):
            print("Percentage of {} : {}".format(key, results_stats_dic[key]))
    if (print_incorrect_dogs and (results_stats_dic['n_correct_dogs'] + results_stats_dic['n_correct_notdogs'])
        ...
        for key in results_dic:
            if ( ( results_dic[key][3] == 1 and results_dic[key][4] == 0 ) or ( results_dic[key][3] == 0 and results_dic[key][4] == 1 ) ):
                print("Misclassified pet label:", results_dic[key][0])
                print("Misclassified classifier label:", results_dic[key][1])
    else:
    ...
    if (print_incorrect_breed and (results_stats_dic['n_correct_dogs'] != results_stats_dic['n_correct_breed'])):
         ...
         for key in results_dic:
             if ( sum(results_dic[key][3:]) == 2 and results_dic[key][2] == 0 ):
                 print("Misclassified breed pet label:", results_dic[key][0])
                 print("Misclassified breed classifier label:", results_dic[key][1])
    else:
        print("\n No misclassified breed assignments") 
